...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    private final void addCount(long x, int check) {
         ...
         * 1. 如果为空，就通过 cas 操作尝试修改 baseCount 变量，对这个变量进行
         *    原子累加操作(做这个操作的意义是：如果在没有竞争的情况下，仍然采用 baseCount 来记录元素个数)
        ...
        if ((as = counterCells) != null ||
             ...
             * 3. 通过 CAS 修改 CounterCell 随机位置的值，如果修改失败说明出现并发情况（这里又用到了一种巧妙的方法），
             *    调用 fullAndCountRandom 在线程并发的时候会有性能问题以及可能会产生相同的随机数,
    ...
    private final void tryPresize(int size) {
        //对 size 进 行修复 ,主 要目 的是防 止传 入的 值不是 一个 2 次幂的 整数 ，然 后通过tableSizeFor 来讲入参转化为离该整数最近的 2 次幂
